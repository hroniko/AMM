...
function MaxLenMaxHeight(v: tree_ptr): tree_ptr;
procedure FindMaxLenMaxDepth(v: tree_ptr);
...
begin
  if v <> nil then
  begin
    len:= MaxLenWay(v); // Определяем длину пути через вершину
    if len > tmpL  then  // Если она больше уже известной, то
    begin
      tmpL:= len;
      tmpN:= v;
    ...
    if len = tmpL then // если же она равна уже известной, то
    begin
      // надо проверить глубину:
      if v^.depth < tmpS then
      begin
        tmpL:= len;
        tmpN:= v;
        tmpS:= v^.depth;
    ...
    FindMaxLenMaxDepth(v^.left); // Идем вниз влево
    FindMaxLenMaxDepth(v^.right); // Идем вниз вправо
  end;
end;
