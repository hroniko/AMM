...
type
  ...
  MassNode = array of array of Integer;

// ФУНКЦИИ И ПРОЦЕДУРЫ
// 1-1) Процедура подсчета количества узлов дерева
procedure SizeOfTree(v: tree_ptr);
procedure CopyOfTree(v: tree_ptr; var arr : Mass);
...
// 32-2) Основная функция поиска узлов, через которые проходят пути максимальной длины
function Find_AllNode_Between_MaxLenWay(v: tree_ptr): tree_ptr;
// 33-2) Вспомогательная процедура обхода дерева и поиска узлов, через которые проходят пути максимальной длины
procedure Find_AllNode_MaxLenWay(v: tree_ptr);
function Find_AllNode_Between_MaxLenWay2(v: tree_ptr): Integer;



// 2016-12-26 Новое Процедура обхода дерева и создания цепочек из его путей от вершины к листу с последующей печатью их:
// Текущий узел, новое дерево (указатель на голову списка), длина текущего пути, длина минимального пути, список в виде строки
// Основная процедура:
procedure FindMinWayFromRoot(v: tree_ptr);
// Вспомогательная процедура:
procedure FindMinWayFromRootToList(v: tree_ptr; count: Integer; min_count: Integer; str_way: string);
// Процедура удаления
procedure DeleteMiddleFromWay(v: tree_ptr; root: tree_ptr; size: Integer);


var i, size1: Integer;
    massiv: array of Integer;
    //massivNode: MassNode;
    massivPosition: Integer;
    massivList: array of Integer; // Массив ключей листьев
    massivSize: array of Integer; // Массив длин путей до листьев
    massivNode: array of Integer; // Массив ключей от вершины до листьев
...
begin
  ...
  begin
    ...
    else if k > p^.key then p^.right:= InsertKey(p^.right, k);
    BalancedDepthWeightHeight(p, 0, 0);
    Result:= p;
  end;
end;
...
// 21) ОСНОВНАЯ Функция ЛЕВОГО удаления элемента по его ключу // ИСПРАВЛЕНО! 2017-01-24
    ...
    if k < p^.key then p^.left:= LeftRemoveKey(p^.left, k)
      ...
      if k > p^.key then p^.right:= LeftRemoveKey(p^.right, k)
        ...
        begin
          ...
          q:= p^.left; // Запомнили левое поддерево   // 10
          r:=p^.right; // Запомнили правое поддерево  // 13
          if q = nil then p:= r   // Если левого поддерева нет, просто переносим на место строго узла новый
          else
          begin
            max:= FindMaxNode(q); // иначе ищем максимальный элемент в левом поддереве и запоминаем его
            // Writeln(min^.key); // 12 Для отладки
            max^.left:= RemoveMaxNode(q); // Перенесли в левое поддерево 12 элемента элемент 13 и всю его цепочку
            // Writeln((min^.right)^.key);  // Для отладки
            max^.right:= r;
...
begin
  ...
  begin
    ...
    size:= CountLR(v);
    SetLength(arr, size+1);  // Выставляем длину одномерного массива
    i:= -1;
...
begin
  ...
  tmpL:= 0; // Считаем длину максимального пути равной 0
  tmpS:= 10000; // Запоминаем в глобальной переменной сумму конечных ключей, пусть она будет очень большой
  Find_AllNode_MaxLenWay(v);  // И запускаем поиск узлов с максимальной длиной
  Result:= tmpN;
end;


// 32-3) Основная функция поиска ключа узла, который является вторым по значению
function Find_AllNode_Between_MaxLenWay2(v: tree_ptr): Integer;
var R : Integer;
    arr : Mass;
begin
  ...
  Find_AllNode_MaxLenWay(v);  // И запускаем поиск узлов с максимальной длиной
    ...
    SizeOfTree(tmpN);
    //Writeln('Size = ', size1);
    SetLength(arr, size1);  // Выставляем длину одномерного массива

    i:= -1;
    CopyOfTree(tmpN, arr); // Выполняем копирование подходящих значений в массив


  QuickSortNonRecursive(arr);  // Сортируем массив по возрастанию
  // и вытаскиваем вторую по значению вершину:
  if (Length(arr)>1) then Result:= arr[1]
  else Result:= -1;
end;
...
begin
  ...
  begin
      i:= i + 1;
      arr[i]:= v^.key; // Скопировать значение ключа
      CopyOfTree(v^.left, arr);
      CopyOfTree(v^.right, arr);
  end;
...
begin
  ...
  begin
    ...
    if len > tmpL  then  // Если она больше уже известной, то
    begin
      // Создаем новое дерево (дерево найденных вершин)
      tmpN:= nil;
      tmpN:= InsertKey(tmpN, v^.key); // И добавляем в него найденный ключ
      //tmpN:= v; // запоминаем узел
      tmpL:= len; // запоминаем новую максимальную длину
    end;
    if len = tmpL then // если же она равна уже известной, то нужно просто добавить новый ключ в дерево найденных вершин:
    begin
      tmpN:= InsertKey(tmpN, v^.key); // добавляем в него найденный ключ
    end;
    Find_AllNode_MaxLenWay(v^.left); // Идем вниз влево
    Find_AllNode_MaxLenWay(v^.right); // Идем вниз вправо
  end;
end;
...
// 2016-12-26 Новое Процедура обхода дерева и создания цепочек из его путей от вершины к листу с последующей печатью их:
// Текущий узел, длина текущего пути, длина минимального пути, список в виде строки
// Основная процедура:
procedure FindMinWayFromRoot(v: tree_ptr);
var i: Integer;
    min: Integer;
begin
  // Создаем массив деревьев (для сохранения в него всех путей):
  SetLength(massivList, 1000);  // Выставляем длину одномерного массива
  SetLength(massivSize, 1000);  // Выставляем длину одномерного массива
  //massivSize
  massivPosition:= 0;
  //Writeln('Все существующие пути от корня до листьев:');
  FindMinWayFromRootToList(v, 0, v^.height + 2 , 'Путь ');

  // Определяем длину минимального пути:
  min:= massivSize[0];
  ...
  else   // Иначе идем и удаляем:
  begin
    // Еще раз Находим в массиве все пути с такой длиной:
    for i:= 0 to massivPosition-1 do
    begin
...
begin
  ...
  begin
    ...
    begin
      ...
      massivList[massivPosition]:= v^.key; // Заносим ключ в массив
      massivSize[massivPosition]:= count; // Заносим длину пути в массив
      massivPosition:= massivPosition + 1;
      FindMinWayFromRootToList(v^.left, count+1, min_count, str_way + v^.info + ' ');
    end
    else
    begin
      FindMinWayFromRootToList(v^.left, count+1, min_count, str_way + v^.info + ' ');
      FindMinWayFromRootToList(v^.right, count+1, min_count, str_way + v^.info + ' ');
    end;
  end
  else
  begin
    //Writeln(str_way, ' - длина пути ', count-1);
  end;
end;
...
begin
  ...
  begin
    ...
    if (v^.depth = size) then  // Если высота и глубина совпадают, то э   ИСПРАВЛЕНО!!! 22-01-2017. ( А было вот так:// if (v^.height-1 = v^.depth) and (v^.height-1 = size)
    begin
      Writeln('Удаляю ', v^.key);
      root:= LeftRemoveKey(root, v^.key);
    end

    else
    begin
      DeleteMiddleFromWay(v^.left, root, size);
      DeleteMiddleFromWay(v^.right, root, size);
    end;
  end;
end;
