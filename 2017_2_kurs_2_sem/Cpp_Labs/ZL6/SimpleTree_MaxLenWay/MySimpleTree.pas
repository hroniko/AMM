...
begin
  ...
  n:= 0;
  SetLength(arr, 100);  // Выставляем длину одномерного массива
  write('Путь максимальной длины: '); PrintMaxLenWay(v);
  ...
  begin
    QuickSortNonRecursive(arr, n); // Сортируем массив
    if (n mod 2) = 0 then
    begin
      Writeln('Узлов в пути ', n, ', четное количество ');
      Writeln('Нет медианы среди найденных узлов. Ничего не удаляем');
    end
    else
    begin
      key:= arr[(n div 2)];
      Writeln('Узлов в пути ', n, ', нечетное количество ');
      Writeln('Средняя по значению вершина: ', key, '. Удаляем ее');
